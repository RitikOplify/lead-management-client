...
//           !leadId ? "" : ""
//         } space-y-6 overflow-y-auto bg-white rounded-xl shadow-sm custom-scroller2`}
//       >
...
import CreatableSelectInput from "../CustomSelect";
import CustomSelect from "../CustomCreatableSelectInner";
import LeadSearchComponent from "../SearchComponent";

function CreateLeadForm({ onClose }) {
  const {
    register,
    handleSubmit,
    control,
  ...
  // Fetch visit details if visitId exists
  useEffect(() => {
    async function getVisitDetails() {
      try {
        const { data } = await axios.get(`/visit/${visitId}`);
  ...
  // Pre-fill form with visit details
  useEffect(() => {
    if (visitDetails) {
      reset({
        customerName: visitDetails?.customer?.customerName || "",
  ...
  useEffect(() => {
    ...
    async function callSearchLeadApi() {
      try {
        ...
        toast.error(error?.response?.data?.message || "Lead search failed");
      }
    }
